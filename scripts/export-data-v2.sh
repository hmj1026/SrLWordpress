...
main() {
    ...
    if docker exec "$MYSQL_CONTAINER" sh -c "command -v pv >/dev/null 2>&1"; then
        docker exec "$MYSQL_CONTAINER" sh -c "mariadb-dump $MYSQLDUMP_OPTS | pv -W -N '資料庫備份' > /tmp/database.sql" \
            && docker cp "$MYSQL_CONTAINER:/tmp/database.sql" "$BACKUP_DIR/database.sql" \
            || error_exit "資料庫備份失敗"
    else
        docker exec "$MYSQL_CONTAINER" mariadb-dump $MYSQLDUMP_OPTS > "$BACKUP_DIR/database.sql" \
            || error_exit "資料庫備份失敗"
        echo "完成（無進度顯示）"
    fi
